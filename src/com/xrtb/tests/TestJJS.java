...
public class TestJJS extends TestCase {
	 ...
	 * 
	 * @throws Exception
	 *             if the script engine fails or the requested classes can't be
	 *             found.
	 */
	@Test
	public void testNashorn() throws Exception {
		...
		try {
			...
			Configuration conf = (Configuration) engine
					.eval("conf = com.xrtb.common.Configuration.getInstance()");
			assertNotNull(conf);
			...
			x = (RTBServer) engine
					.eval("Server = new com.xrtb.bidder.RTBServer()");
			Echo m = x.getStatus();
			assertFalse(m.stopped);

			CampaignSelector camps = (CampaignSelector) engine
					.eval("camps = Server.getCampaigns()");
			assertEquals(camps.size(), 1);

			/**
			 * Now let's test a bid request
			 */
			BidRequest br = (BidRequest) engine
					.eval("br = new com.xrtb.exchanges.Nexage(\"SampleBids/nexage.txt\")");
			assertEquals(br.getId(), "35c22289-06e2-48e9-a0cd-94aeb79fab43");

			// Now let's bid...
			BidResponse response = (BidResponse) engine
					.eval("response = camps.get(br)");
